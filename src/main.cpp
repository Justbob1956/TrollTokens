...
unsigned int nTargetSpacing = 3 * 60; // TrollTokens - 4 minute blocks
...
const string strMessageMagic = "TrollTokens Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight+1 <= 2)
    {
        nSubsidy = 100000 * COIN;
	     ...
	     else if(pindexBest->nHeight+1 >= 3  && pindexBest->nHeight+1 <= 201)
    {
        nSubsidy = 2 * COIN;
    }
        else if(pindexBest->nHeight+1 >= 202  && pindexBest->nHeight+1 <= 1001)
    {
        nSubsidy = 10 * COIN;
    }
	    else if(pindexBest->nHeight+1 >= 1002  && pindexBest->nHeight+1 <= 10001)
    {
        nSubsidy = 15 * COIN;
    }
	    else if(pindexBest->nHeight+1 >= 10002  && pindexBest->nHeight+1 <= 20001)
    {
        ...
        else if(pindexBest->nHeight+1 >= 20001)
    {
        nSubsidy = 1 * COIN;
    }
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // TrollTokens: check proof-of-stake block signature
...
// TrollTokens: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TrollTokens", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "test sha256 staker";
        CTransaction txNew;
        txNew.nTime = 1451185936;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1451185936;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        //block.nNonce   = 1709179;
        block.nNonce   = 2283707;
...
*/
        ...
        assert(block.hashMerkleRoot == uint256("0x987c2a0da4284134521e9f84813d46e6de4e010594ccbbe373082df4e303272c")); 
...
unsigned char pchMessageStart[4] = { 0xa4, 0xd2, 0xf8, 0xa4 };
